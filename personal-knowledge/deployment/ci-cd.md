...
tags: [deployment, ci-cd, dockerfile, vercel, webhooks]
...
1. **Register the repo** manually in Vercel's dashboard.
...
Every deployable repository maintains a `Dockerfile` by default. It is the authoritative build recipe for local and hosted deployments. Any container-capable platform must build from that Dockerfile rather than infer a platform-native build configuration.
